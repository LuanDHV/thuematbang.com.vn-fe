...
import { LISTING_STATUS_OPTIONS } from "../../lib/admin/constants/options";
import {
  buildAdminSearchFilters,
  submitAdminSearch,
} from "../../lib/admin/utils/table";
import { formatLocationParts } from "@thuematbang/contracts";

export const RentRequestsList: React.FC = () => {
  const location = useLocation();
  const { tableProps, searchFormProps, setCurrentPage, setFilters } = useTable({
    resource: "rent-requests",
    pagination: {
      mode: "server",
    },
    sorters: {
  ...
  return (
    <>
      ...
      <AdminDataTable tableProps={tableProps} minWidth={1000}>
        ...
        <Table.Column
          ...
          render={(_: unknown, record: Record<string, unknown>) =>
            formatLocationParts([
              (record.desiredWard as Record<string, unknown> | undefined)
